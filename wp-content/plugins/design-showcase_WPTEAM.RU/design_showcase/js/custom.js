...
$templateurl = jQuery("meta[name=DS_option1]").attr('content');
$toolimg = jQuery("meta[name=DS_option2]").attr('content');
$tooltitle = jQuery("meta[name=DS_option3]").attr('content');
$smoothscrolling = jQuery("meta[name=DS_option4]").attr('content');
$footer = jQuery("meta[name=DS_option5]").attr('content');
$tabs = jQuery("meta[name=DS_option6]").attr('content');
...
jQuery(selector).each(function(i){
			...
			jQuery("body").append("<div class='"+selectname+"' id='"+selectname+i+"'><div><img class='tooltipimg' src='"+jQuery(this).attr(atrribute)+"' alt='' /></div><span><img src='" +$templateurl+ "/images/tooltip-trans.png' alt='' /></span></div>");
...
jQuery(document).ready(function(){
	...
	kriesi_search_fade();
	kriesi_mainmenu();
    //if ($smoothscrolling) my_smothscroll(); 
    if ($toolimg) kriesi_tooltip('.prev_image a', 'tooltip_image', 'rel');
    if ($tooltitle) kriesi_tooltip2('a', 'tooltip_image2', 'title');
    if ($tabs)  kriesi_tab('.widget_block','.widgettitle','.widget');
    if ($footer)  kriesi_footer();
});
