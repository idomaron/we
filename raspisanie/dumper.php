...
include ('config.php');
	...
	//Удаление файлов бэкапа, если их больше 3
	$backup=array();
    foreach (glob("backup/*.gz") as $filename) 
	{
		$bb++;
		$backup[$bb]=$filename;
    }
	...
	for ($f=0;$f<$bb;$f++)
	{
		if ($f>=$kolvo_soxr) unlink($backup[$f]);;
	}

	

//Иногда просто ошибка вылазила
...
class dumper {
	...
	function backup() {
		...
		$buttons = "<A ID=save HREF='' STYLE='display: none;font-size:12px;'>Скачать файл</A> &nbsp; <INPUT ID=back TYPE=button VALUE='Вернуться' DISABLED onClick=\"history.back();\">";
	...
	function main(){
		...
		$buttons = "<INPUT TYPE=submit VALUE=Применить><INPUT TYPE=button VALUE=Выход onClick=\"window.close();\">";
...
<LEGEND>
<label title='Сохранить базу данных'>
<INPUT TYPE=radio NAME=action VALUE=backup>
<b>Создание резервной копии БД</b>&nbsp;</label></LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD WIDTH=35%>БД:</TD>
...
<label title="Восстановить базу данных из ранее сохранённых.">
<b>Восстановление БД из резервной копии</b>&nbsp;</label></LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD>БД:</TD>
